...
- 负责 `OceanProtect` 管控面在保护引擎、系统管理和基础平台等团队间的子系统架构设计与技术路线规划。
- 协同三支团队推进跨模块架构决策、接口边界治理、质量门禁与疑难问题攻关，在高风险需求和现网问题中承担技术负责人角色。
- 设计并落地一套面向基础设施及管理 / 控制平面 `DEG` 评审的人类在环、Agent 驱动的架构决策治理流程。
...
- 主导 **22+** 项性能优化与重构，覆盖备份、查询、导出和事件处理链路。代表性成果包括：10,000 台虚拟机扫描从 **2 小时** 缩短到 **10 分钟**，10,000 个副本归档队列 `Redis` 内存从 **2 GB** 降至 **56 MB**，批量取消从 **1 分钟/任务** 降至 **0.5 秒/任务**，400 万条告警/事件转储从 **24 小时** 缩短到 **35 秒**。
...
- 作为部门 `Java` / `Python` Committer，推动三支团队的代码评审和缺陷预防，累计输出 **621+** 条检视意见，识别 **15** 个安全问题和 **5** 个数据库性能问题，并培养 **5** 名 Tech Lead。
- 证书
...
- 识别重复验证与人工回归瓶颈，使用 `PyTest`、`scrapy` 和仪表板搭建自动化测试平台，并通过内部 RESTful API 集成进团队日常敏捷流程。
...
## `OceanProtect` 管控面与数据保护平台

**华为 - 管控业务团队**

- 项目角色：管控面子系统架构 owner、跨团队技术负责人、AI 增强 `DEG` 治理联合主持人。
- `OceanProtect` 是华为面向下一代数据中心与多云环境的数据保护产品，在统一平台上整合备份、恢复、副本管理与网络韧性能力，支撑边缘、中心到多云的全场景数据保护。
- 在一体机形态下，产品覆盖备份软件、备份服务器、备份存储、数据管理与安全能力，例如 `WORM`、防删除与 `Air Gap`，因此管理 / 控制平面与保护引擎协同是整套系统设计的关键。
- 我的贡献首先体现在作为管控面子系统架构 owner，统筹保护引擎、系统管理和基础平台等团队之间的技术路线、接口边界、方案决策与质量门禁，体现跨团队架构设计与技术管理能力。
- 我还引入并联合主持 AI 增强的 `DEG` 架构治理机制，在明确人类决策 owner 的前提下，引入 `GPT`、`Gemini`、`Qwen`、`DeepSeek` 作为结构化质询者，在实现前提前暴露可扩展性、安全性与长期可维护性风险。
- 具体落地上，我主导了调度器、副本归档队列 `Redis` 内存与告警 / 事件链路重构，并交付 `MPA` 等合规控制能力以及 `Nutanix` 备份插件等生态集成。

## `ECS` / `ObjectScale` 分布式对象存储平台

**Dell Technologies - Dell EMC**

- 项目角色：Scrum Lead、跨地域技术协调者，以及元数据、写入链路与 `DT` 自动化 / 诊断方向的工程负责人。
- `ECS` 是 Dell 面向企业级对象存储的平台，具备基于 S3 的全球分布式架构、统一全局命名空间，以及面向 AI、分析、备份、归档等大规模非结构化数据场景的可扩展性、安全性与经济性。
- `ObjectScale` 则在 `ECS` 基础上进一步演进为现代化、基于 `Kubernetes` 的对象存储架构，强调 AI-ready 数据管理，并在产品线收敛过程中延续 `ECS` 的核心工作流、自动化能力与 API 体系。
- 我的贡献聚焦于元数据与写入链路的跨地域交付、`DT` 写入优化、存储数据结构设计、`DT Automation` / `Fortress-Diag` 自动化诊断能力，以及从 `VMware` 版 `ECS` 向 `Kubernetes` 版 `OBS` / `ObjectScale` 的工程演进。
...
- 设计一种自递归、成本可控的对象存储架构，使用户可通过统一接口访问多种存储后端。
- 该方案同时支持单例部署形态，例如云盘融合服务，也支持集群形态，即对象存储云既能持有本地数据，也能将存储委托给其他云服务。
- 认可
...
- 项目角色：`amdgpu-pro` / `GIM` 虚拟化交付与验证改进的核心开发者与执行 owner。
- AMD 的 GPU 虚拟化技术栈建立在 `MxGPU` / `SR-IOV` 之上，使多个虚拟机能够在 `QEMU` / `KVM` 环境中高效共享加速卡资源，并兼顾生产级隔离性、分区能力与性能表现。
- 在这套技术栈中，`amdgpu-pro` 与 `GIM` 贴近内核态驱动、GPU 分区、渲染调度与虚拟化验证链路，因此底层执行质量与自动化测试覆盖对交付尤为关键。
- 我的贡献聚焦于 `amdgpu-pro` / `GIM` 相关的 Linux 驱动与虚拟化特性开发、高数量问题单的端到端闭环、通过状态机协调 VM 渲染请求，以及提升整套栈的自动化验证覆盖与通过率。
...
- NI 官方测试软件体系的核心语境是自动化验证与生产测试，像 `TestStand`、`SystemLink` 这样的产品强调自动执行、集中式看板、可追溯性、API 集成，以及跨分布式测试系统的软件部署与运维。
- 这个内部项目与上述测试运营语境高度一致，围绕自动化执行、数据采集、可视化看板、内部 RESTful API 以及 `OneRT` Linux feed / package 管理，构建面向研发日常使用的轻量级验证平台。
- 我的贡献聚焦于识别重复验证瓶颈，基于 `PyTest`、`scrapy`、看板与 API 搭建后端自动化流程，并改进 `OneRT` feed 与 bundle 管理，以减少人工回归成本和日常运维摩擦。
...
- 从需求到实现独立构思并开发后端，基于 `Flask` 与 `Docker` 接入公司微信小程序，并通过 RESTful API 串联业务流程，帮助降低 **10%** 的资产损失成本。
- 认可
    1. _2019 2nd Most Popular National Instruments Tech Week Project_
