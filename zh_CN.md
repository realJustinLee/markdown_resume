...
> 高级工程师 A **17B** | 华为 | [justindelladam@live.com](mailto:justindelladam@live.com) | +86 180 1601 5760 | 中国上海 | 1997 年 9 月 17 日

**以架构为导向的高级工程师，拥有 6 年以上云存储与数据保护领域的大规模分布式系统设计与演进经验。在高并发、严苛可靠性要求及合规约束条件下，多次承担并主导关键架构决策，具备成熟、可落地的架构判断能力。**

- 英语流利，雅思 **7.5** 分。
- 中国计算机学会专业会员。
- 核心专长聚焦于**分布式系统架构**、**管控面设计**以及**性能治理**。
...
# 技能
- **AI 增强的架构决策治理**
    - 设计并落地了一套**人类 + AI 混合的架构决策流程**，用于覆盖基础设施以及管理与控制平面的 DEG 会议决策场景。
    - 作为**人类决策负责人及联合主持人**，有意识地引入 AI Agent（`GPT`、`Gemini`、`Qwen`、`DeepSeek`）作为结构化的质询者，用于揭示系统在可扩展性、安全性及长期可维护性方面的潜在风险。
    - 明确决策归属，系统性记录关键取舍，并建立决策后的验证机制，显著降低了架构盲区，在实现前有效避免了多起高风险架构回退与设计缺陷。
- **架构设计**
    - 面向云原生平台的管控面及分布式系统架构设计
    - 在规模、性能与合规约束条件下的架构决策制定
    - 系统性能、可扩展性与可靠性治理
    - 跨团队架构评审与技术决策责任承担
...
- 担任`管控业务团队`的子系统架构师、特性 SE、DEG Leader，`保护引擎组`的 DE、FO，`系统管理组`的 DE，`基础平台组`的 DE：端到端看护核心需求分析、方案设计、疑难问题攻关和质量控制
    1. 编码绝对贡献
        - 上`master`库代码总量：54.5K+ 行。
        - 输出检视意见：621+ 个`issue`。
    2. 修改、处理疑难、性能问题单 152+ 个。
    ...
    11. 提出 9+ 项架构整改建议，主导 22+ 项性能优化
        ...
        - 设计重构备份代理心跳上报逻辑，提升单个一体机 Agent 管理个数 **1,000%**.
- 担任`数据保护 PDT` RAT 备份软件管控面需求分析专家、`管控业务团队`特性 SE
    1. 看护`管控业务团队`项目方案设计，即时完成 RAT 110 + 待分析/决策需求的分析与决策。
...
- 提出 9+ 项架构整改建议，主导 22+ 项性能优化
    ...
    11. 设计重构备份代理心跳上报逻辑，提升单个一体机 Agent 管理个数 **1,000%**.
